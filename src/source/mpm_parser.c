...
static void mpm_init_impl(const char* boundary, size_t boundary_size,
    ...
    out_context->boundary_size =
        boundary_size == 0 ? strlen(boundary) : boundary_size;
    ...
    while (1) {
        switch ((mpm_state_t)ctx->state) {
            ...
            case MPM_S_HEADER_VALUE_END:
                ...
                ctx->state = (int)MPM_S_HEADER;
                break;
            case MPM_S_HEADER_VALUE_END_THEN_HEADER_END:
            ...
            case MPM_S_CONTENT:
                ...
                ctx->i = ctx->on_read(ctx->read_state);
                break;
            case MPM_S_CONTENT_THEN_HEADER:
                ctx->state = (int)MPM_S_HEADER;
                ctx->skip_next_read = 1;
                return MPM_CONTENT_END;
            case MPM_S_CONTENT_THEN_END:
                ctx->state = (int)MPM_S_END;
                return MPM_CONTENT_END;

            case MPM_S_MAYBE_BOUNDARY:
                if (ctx->boundary_pos == ctx->boundary_size) {
                    if (ctx->i != '\r' && ctx->i != '-') {
                        ctx->boundary_pos = -4;
            ...
            case MPM_S_COPY_BOUNDARY_PART:
                ...
                if (ctx->boundary_pos < 0) {
                    ((char*)buffer)[(*in_out_size)++] =
                        boundary_start_table[ctx->boundary_pos + 4];
                    --size;
                } else {
                    if (ctx->boundary_pos < ctx->boundary_size) {
                        ((char*)buffer)[(*in_out_size)++] =
                            ctx->boundary[ctx->boundary_pos];
                        --size;
                    } else if (ctx->boundary_pos == ctx->boundary_size + 1) {
                        if (ctx->i == '-') {
                            ((char*)buffer)[(*in_out_size)++] = '-';
